...
The user owns one Chameleon Ultra and a Mac. The executor cannot run hardware steps. At each handoff below, stop, present the exact checklist and commands, and wait for the user's report before continuing. Everything not blocked by the handoff continues.
...
- H2 (during Phase 8, per spec 5.6): USB DFU on macOS with a release package. Then a deliberately interrupted BLE DFU recovered over USB. Only then enable BLE DFU, then iOS DFU.
...
| 3 Transports | write from spec 5, 4.1, 4.2 | 5 | `packages/chameleon_flutter`: BLE transport, serial transport, scanners, BleDfuChannel, SlipSerialDfuChannel, permission and pairing states, platform setup files, contract suite | contract suite green against FakeDevice; H1 handoff issued |
...
| 8 Firmware update | write from spec 4.5, 5.3, 5.5, 5.6, 7.7 step 6 | 4.5, 5.6 | Release feed, package pick, orchestrated DFU UI, recovery flow; delivered USB first per 5.6 | H2 handoff; iOS DFU enabled last |
...
| 10 Release | write from spec 10 | 10 | Signing, notarization, installers, AppImage, changelog, hardware checklist run | H3 handoff; tagged release artifacts |
